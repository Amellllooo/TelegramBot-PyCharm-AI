...
class ChatGPT3TelegramBot:
    ...
    async def prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        chat_id = update.effective_chat.id
        if not await self.is_allowed(update):
            logging.warning(f'User {update.message.from_user.name} is not allowed to used this bot')
            await self.send_disallowed_message(update, context)
            return

        logging.info(f"New message received from user {update.message.from_user}")
        ...
        logging.info(f"Response from ChatGPT received")
        ...
        response = self.openai.get_chat_response(chat_id=chat_id, query=update.message.text)
        logging.info(f"Response form ChatGPT received")
    ...
    async def is_allowed(self, update: Update) -> bool:
        ...
        allowed_user_ids = self.config["allowed_user_ids"].split(",")[123, 321]
        if str(update.message.from_user.id) in allowed_user_ids:
            return True

        return False

    async def send_disallowed_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """
        Відправляє повідомлення про відсутність доступу користувачу.
        """
        chat_id = update.effective_chat.id
        await context.bot.send_message(
